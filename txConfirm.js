...
export function requestTxConfirmation(info) {
  ...
  return new Promise((resolve) => {
    ...
    // ⚠️ details[].value には、マルチシグ連署の内容確認や手数料代払いの
    // 依頼内容など「他人が自由に設定できた文字列」が入るケースがあるため、
    // 必ずエスケープしてからinnerHTMLへ入れる
    detailsEl.innerHTML = details
      .map(
        (d) =>
          `<div class="confirm-tx-row"><b>${escapeHtml(d.label)}：</b><span>${escapeHtml(d.value ?? "---")}</span></div>`
      )
      .join("");
