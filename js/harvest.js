...
async function announcePersistentDelegationRequest(remoteKeyPair, vrfKeyPair, nodePublicKey) {
  ...
  let encodedMessage;
  try {
    const dummyKeyPair = new appState.sdkCore.KeyPair(randomPrivateKey());
...
export async function startHarvest() {
  ...
  try {
    ...
    const remoteKeyPair = new appState.sdkCore.KeyPair(randomPrivateKey());
    const vrfKeyPair = new appState.sdkCore.KeyPair(randomPrivateKey());
